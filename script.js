const projectData = [
  ...
  {
    title: 'Food & Restaurant Analytics',
    count: 15,
    items: [
  ...
  {
    title: 'Advanced / Portfolio-Level Projects',
    count: 20,
    items: [
...
const totalProjects = projectData.reduce((sum, category) => sum + category.items.length, 0);

document.getElementById('category-count').textContent = projectData.length;
document.getElementById('project-count').textContent = totalProjects;
...
projectData.forEach((category) => {
  ...
  badge.textContent = `${category.items.length}`;
